---
title: "Sesgo y Curtosis"
output: 
  flexdashboard::flex_dashboard:
    orientation: columns
    vertical_layout: fill
---

```{r setup, include=FALSE}
library(flexdashboard)
library(quantmod)
library(moments)
library(plotly)
library(MASS)
library(quantmod)
library(moments)
library(plotly)


library(MASS)



# Define stock symbols and date range
stock_symbols <- c("KO", "TSLA", "BA", "JPM", "MMM")
start_date <- "2021-01-01"
end_date <- Sys.Date()  # Assuming you want data up to the current date

# Fetch stock price data
stock_data <- lapply(stock_symbols, function(symbol) {
  getSymbols(symbol, from = start_date, to = end_date, auto.assign = FALSE)
})


returns_data <- lapply(stock_data, function(stock) {
  daily_returns <- diff(log(Cl(stock)))
  colnames(daily_returns) <- gsub("\\.[A-z]*", "", colnames(daily_returns))
  return(daily_returns)
})

# Remove the first row from each stock's returns
returns_data <- lapply(returns_data, function(returns) {
  tail(returns, -1)
})


skewness_results <- sapply(returns_data, function(returns) {
  calculate_skewness <- function(x) {
    mean_x <- mean(x)
    sd_x <- sd(x)
    skewness <- sum((x - mean_x)^3) / length(x) / (sd_x^3)
    return(skewness)
  }
  
  skewness <- sapply(returns, calculate_skewness)
  return(skewness)
})

kurtosis_results <- sapply(returns_data, function(returns) {
  calculate_kurtosis <- function(x) {
    mean_x <- mean(x)
    sd_x <- sd(x)
    kurtosis <- sum((x - mean_x)^4) / length(x) / (sd_x^4) - 3
    return(kurtosis)
  }
  
  kurtosis <- sapply(returns, calculate_kurtosis)
  return(kurtosis)
})

# Function to fit a t-distribution to returns
fit_t_distribution <- function(returns) {
  fit <- suppressWarnings(fitdistr(returns, densfun = "t"))
  df <- fit$estimate["df"]
  mean <- fit$estimate["mean"]
  sd <- fit$estimate["sd"]
  return(list(df = df, mean = mean, sd = sd))
}

# Fit t-distributions to each stock's returns
t_distribution_params <- lapply(returns_data, fit_t_distribution)

# Example: Weighting by skewness 
skewness_weights <- skewness_results / sum(skewness_results)

kurtosis_weights <- kurtosis_results / sum(kurtosis_results)

combined_weights<-(skewness_weights+kurtosis_weights)/sum(skewness_weights+kurtosis_weights)

# Create the portfolio
portfolio <- lapply(seq_along(returns_data), function(i) {
  returns <- returns_data[[i]]
  weight <- combined_weights[i]
  return(returns * weight)
})

# Combine the portfolio returns
portfolio_returns <- rowSums(do.call(cbind, portfolio))


# Create a function to plot portfolio weights using plotly
plot_portfolio_weights_plotly <- function(stock_weights, stock_symbols) {
  # Create a plotly bar chart
  plot <- plot_ly(x = stock_symbols, y = stock_weights, type = "bar", name = "Portfolio Weights", marker = list(color = "green")) %>%
    layout(title = "Portfolio Weights",
           xaxis = list(title = "Stocks"),
           yaxis = list(title = "Weight"))
  
  return(plot)
}

# Create a portfolio weights plot using plotly
portfolio_weights_plot <- plot_portfolio_weights_plotly(weights, stock_symbols)
```

Portafolio
=======================================================================

Column {data-width=650}
-----------------------------------------------------------------------

### Análisis de Sesgo y Curtosis

```{r}
# Create a bar chart for skewness and kurtosis
skew_kurtosis_chart <- plot_ly(x = stock_symbols) %>%
  add_trace(y = skewness_results, name = "Skewness", type = "bar", marker = list(color = "blue")) %>%
  add_trace(y = kurtosis_results, name = "Kurtosis", type = "bar", marker = list(color = "red")) %>%
  layout(title = "Skewness and Kurtosis Comparison",
         xaxis = list(title = "Stocks"),
         yaxis = list(title = "Value"))

# Create a line chart for the portfolio returns
portfolio_returns_chart <- plot_ly(x = index(returns_data[[1]]), y = cumsum(portfolio_returns), type = "scatter", mode = "lines", name = "Portfolio Returns") %>%
  layout(title = "Portfolio Returns Based on Skewness-Weighted Allocation",
         xaxis = list(title = "Date"),
         yaxis = list(title = "Portfolio Returns"))

# Display the plots
subplot(skew_kurtosis_chart, portfolio_returns_chart, nrows = 2)
```
Column {data-width=450}
-----------------------------------------------------------------------
### VeR del portafolio

```{r}

medrend=mean(portfolio_returns) # media de los rendimientos
volrend=sd(portfolio_returns)## volatilidad de los rendimiento
pport=cbind(tail(as.numeric(stock_data[[1]]$KO.Close),1),tail(as.numeric(stock_data[[2]]$TSLA.Close),1),tail(as.numeric(stock_data[[3]]$BA.Close),1),tail(as.numeric(stock_data[[4]]$JPM.Close),1),tail(as.numeric(stock_data[[5]]$MMM.Close),1))
pport=sum(skewness_weights*pport)
inv=100*pport

VeR1=inv*qnorm(0.01)*volrend*sqrt(20) ### VaR=Inversión*Valor Z de la distribución normal* Volatilidad
VeR5=inv*qnorm(0.05)*volrend*sqrt(20)
VeR10=inv*qnorm(0.1)*volrend*sqrt(20)
sim=rnorm(10000,inv,inv*volrend*sqrt(20))

hist(sim, breaks=100, col="blue", main="Valor en Riesgo del portafolio en 20 días")
abline(v=inv+inv*volrend*sqrt(20), col="red")
abline(v=inv-inv*volrend*sqrt(20), col="red")
abline(v=inv+inv*qnorm(0.01)*volrend*sqrt(20), col="green")
abline(v=inv+inv*qnorm(0.05)*volrend*sqrt(20), col="yellow")
abline(v=inv+inv*qnorm(0.1)*volrend*sqrt(20), col="black")


```

Densidad KO
=======================================================================


Column {data-width=500}
-----------------------------------------------------------------------

### Valor en Riesgo

¿Cuanto es lo máximo que podrìa llegar a perder como resultado de una caida del precio de la acción si yo compro 100 acciones por 20 días?

```{r}

medrend=mean(returns_data[[1]]) # media de los rendimientos
volrend=sd(returns_data[[1]])## volatilidad de los rendimiento

inv=100*as.numeric(tail(stock_data[[1]]$KO.Close,1))

VeR1=inv*qnorm(0.01)*volrend*sqrt(20) ### VaR=Inversión*Valor Z de la distribución normal* Volatilidad
VeR5=inv*qnorm(0.05)*volrend*sqrt(20)
VeR10=inv*qnorm(0.1)*volrend*sqrt(20)
VeR1
VeR5
VeR10

```

### VaR con Inversión 20 días

```{r}
### simulación 
sim=rnorm(10000,inv,inv*volrend*sqrt(20))

hist(sim, breaks=100, col="blue", main="Valor en Riesgo de KO en 20 días")
abline(v=inv+inv*volrend*sqrt(20), col="red")
abline(v=inv-inv*volrend*sqrt(20), col="red")
abline(v=inv+inv*qnorm(0.01)*volrend*sqrt(20), col="green")
abline(v=inv+inv*qnorm(0.05)*volrend*sqrt(20), col="yellow")
abline(v=inv+inv*qnorm(0.1)*volrend*sqrt(20), col="black")



```




Column {data-width=500}
-----------------------------------------------------------------------

### Grafico

```{r}
### simulación 
sim=rnorm(10000,medrend,volrend)

hist(sim, breaks=100, col="blue", main="Valor en Riesgo de KO un día")
abline(v=medrend-volrend, col="red")
abline(v=medrend+volrend, col="red")
abline(v=medrend+qnorm(0.01)*volrend, col="green")
abline(v=medrend+qnorm(0.05)*volrend, col="yellow")
abline(v=medrend+qnorm(0.1)*volrend, col="black")


```


### VaR con Inversión

```{r}
### simulación 
sim=rnorm(10000,inv,inv*volrend)

hist(sim, breaks=100, col="blue", main="Valor en Riesgo de KO un día")
abline(v=inv+inv*volrend, col="red")
abline(v=inv-inv*volrend, col="red")
abline(v=inv+inv*qnorm(0.01)*volrend, col="green")
abline(v=inv+inv*qnorm(0.05)*volrend, col="yellow")
abline(v=inv+inv*qnorm(0.1)*volrend, col="black")



```

Densidad TSLA
=======================================================================


Column {data-width=500}
-----------------------------------------------------------------------

### Valor en Riesgo
Obtenga la densidad a 20 días de las demás acciones

```{r}

medrend=mean(returns_data[[2]]) # media de los rendimientos
volrend=sd(returns_data[[2]])## volatilidad de los rendimiento

inv=100*as.numeric(tail(stock_data[[2]]$TSLA.Close,1))

VeR1=inv*qnorm(0.01)*volrend*sqrt(20) ### VaR=Inversión*Valor Z de la distribución normal* Volatilidad
VeR5=inv*qnorm(0.05)*volrend*sqrt(20)
VeR10=inv*qnorm(0.1)*volrend*sqrt(20)
VeR1
VeR5
VeR10

```

### VaR con Inversión 20 días

```{r}
### simulación 
sim=rnorm(10000,inv,inv*volrend*sqrt(20))

hist(sim, breaks=100, col="blue", main="Valor en Riesgo de TSLA en 20 días")
abline(v=inv+inv*volrend*sqrt(20), col="red")
abline(v=inv-inv*volrend*sqrt(20), col="red")
abline(v=inv+inv*qnorm(0.01)*volrend*sqrt(20), col="green")
abline(v=inv+inv*qnorm(0.05)*volrend*sqrt(20), col="yellow")
abline(v=inv+inv*qnorm(0.1)*volrend*sqrt(20), col="black")



```




Column {data-width=500}
-----------------------------------------------------------------------

### Grafico

```{r}
### simulación 
sim=rnorm(10000,medrend,volrend)

hist(sim, breaks=100, col="blue", main="Valor en Riesgo de TSLA un día")
abline(v=medrend-volrend, col="red")
abline(v=medrend+volrend, col="red")
abline(v=medrend+qnorm(0.01)*volrend, col="green")
abline(v=medrend+qnorm(0.05)*volrend, col="yellow")
abline(v=medrend+qnorm(0.1)*volrend, col="black")


```


### VaR con Inversión

```{r}
### simulación 
sim=rnorm(10000,inv,inv*volrend)

hist(sim, breaks=100, col="blue", main="Valor en Riesgo de TSLA un día")
abline(v=inv+inv*volrend, col="red")
abline(v=inv-inv*volrend, col="red")
abline(v=inv+inv*qnorm(0.01)*volrend, col="green")
abline(v=inv+inv*qnorm(0.05)*volrend, col="yellow")
abline(v=inv+inv*qnorm(0.1)*volrend, col="black")



```




