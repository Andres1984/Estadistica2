...
bkr <- read_csv("~/Downloads/GEM 1999 NES GLOBAL National Level Data/data-2.csv")
...
Obtenemos los parámetros poblacionales
...
p<-ggplot(bkr, aes(x=bancarota, y=`ROA(C) before interest and depreciation before interest`, fill=bancarota)) +
   geom_boxplot(outlier.shape = NA)+coord_cartesian(ylim = quantile(bkr$`ROA(A) before interest and % after tax`, c(0, 0.9)))
...
```{r}
p<-ggplot(bkr, aes(x=bancarota,y=`Operating Gross Margin`, color=bancarota)) + geom_boxplot(outlier.shape = NA)+coord_cartesian(ylim = quantile(bkr$`Operating Gross Margin`, c(0.1, 0.9)))
...
```{r}
p<-ggplot(bkr, aes(x=bancarota,y=`Cash flow rate`, color=bancarota)) +geom_boxplot(outlier.shape = NA)+coord_cartesian(ylim = quantile(bkr$`Cash flow rate`, c(0.1, 0.9)))
...
```{r}
p<-ggplot(bkr, aes(x=bancarota, y=bkr$`Net Value Per Share (A)`, color=bancarota)) + geom_boxplot(outlier.shape = NA)+coord_cartesian(ylim = quantile(bkr$`Net Value Per Share (A)`, c(0.05, 0.9)))
